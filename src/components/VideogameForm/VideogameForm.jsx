 ...
 <h4>CREATE YOUR GAME</h4>
  ...
  <select 
      ...
      onChange={handleInputOnChangeList}
       >
        <option disabled selected={clicked.platforms}>Choose Platforms</option>
            {platformsForm?.map(plat => <option value={plat.id} key={plat.id} >{plat.name}</option>)}    
  </select>
  </div>
  <div>
  <select 
  ...
  name='Genres' 
  onChange={handleInputOnChangeList}>
    <option disabled selected={clicked.genres}>Choose Genres</option>
          {genresForm?.map(gen => 
          <option 
          value={gen.id} 
          key={gen.id} >{gen.name}</option>)}    
    </select> 
    </div>
    </div>
  </div>
  ...
  <img 
    src={game.background_image || imgForm } 
    alt='game img'/>
    <div className={style.containerPlatGen}>
      <div>
          <ul>
          {game?.platformsGame.map(p => {
            let game = platformsForm.find(ele => ele.id === parseInt(p))
            return <li key={`${p}key`}>{game.name}
            <button type='button'  value={p} onClick={deleteFromList}>X</button>
            </li>
          }
          )}
          </ul>
    ...
    <div>
        ...
        {game?.genresGame.map(g => { 
          ...
          return <li key={`${g}key`}>{game.name}
            <button type='button' value={g} onClick={deleteFromList}>X</button>
            </li>
          })}
        </ul>
  </div>
       ...
       <button className={style.btnForm} value="submit" type="submit"><div className={style.svgWrapper}>
